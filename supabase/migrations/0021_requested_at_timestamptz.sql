...
-- Recreate seed_dev_fixtures using requested_at (previously used dropped column requested_date).
...
begin
  ...
  insert into public.service_requests (senior_id, category, priority, requested_at, description, status, created_by)
  select s.id, 'transportation', 'normal', (current_date + 2)::timestamp at time zone 'America/Toronto', 'Ride to medical appointment', 'open', seed_admin_id
  ...
  insert into public.service_requests (senior_id, category, priority, requested_at, description, status, created_by)
  select s.id, 'shopping', 'normal', (current_date + 3)::timestamp at time zone 'America/Toronto', 'Grocery pickup', 'open', seed_admin_id
  from public.seniors s
  ...
  insert into public.service_requests (senior_id, category, priority, requested_at, description, status, created_by)
  select s.id, 'companionship', 'low', (current_date + 5)::timestamp at time zone 'America/Toronto', 'Weekly visit', 'notified', seed_admin_id
  from public.seniors s
  ...
  insert into public.service_requests (senior_id, category, priority, requested_at, description, status, completed_at, created_by)
  select s.id, 'meal_delivery', 'normal', (current_date + 1)::timestamp at time zone 'America/Toronto', 'Meal drop-off',
  ...
  insert into public.service_requests (senior_id, category, priority, requested_at, description, status, completed_at, assigned_volunteer_id, created_by)
  select s.id, 'household_tasks', 'normal', (current_date - 3)::timestamp at time zone 'America/Toronto', 'Helped change light bulbs',
